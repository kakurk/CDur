...
# ANOVA
aov_car(DF.effect ~ AgeGroup * enctype + Error(subject/enctype), 
        data = df.DFeffects, 
        factorize = T, 
        include_aov = T) -> ANOVA
...
```{r, message=FALSE}
# Are the mean DF-effects within each Age Group significantly different from 0?
emmeans(ANOVA$aov, ~ AgeGroup) %>% 
  test()
```
...
require(emmeans)
emmeans(ANOVA$aov, ~ enctype + AgeGroup) -> EMMEANS.DF
...
```{r}
# Is there a linear effect, a quadratic effect, or a 'pleatau' effect?
...
ANCOVA <- aov_car(DF.effect ~ enctype*composite + Error(subject/enctype), 
                  data = df.DFeffects %>% filter(AgeGroup == 'Old'), 
                  factorize = F, 
                  include_aov = T)
...
# ANOVA
ANOVA <- aov_car(Rec.effect ~ AgeGroup*enctype + Error(subject/enctype), data = df.RecEffects)
...
emmeans(ANOVA$aov, ~ enctype + AgeGroup) -> EMMEANS.RE
...
contrast(EMMEANS.RE, interaction = c('custom_con', 'consec'))
```
