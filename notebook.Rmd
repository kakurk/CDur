...
contrast(EMMEANS.RE, method = 'poly',
         by = 'AgeGroup')
```
...
```{r}
# A custom contrast, comparing one and three seconds conditions with the five second condition
custom_con.emmc <- function(levels){
...
order <- c('SYMBOLSEARCH', 'DIGITSYMBOL', 'DIGITSPAN', 'ARITHMETIC', 'LNSEQUENCING', 'VOCAB', 'MMSE', 'AGE', 'DF.effect')
...
```{r}
ggplot(df.FArates, aes(x = AgeGroup, y = FARate, fill = AgeGroup)) +
  stat_summary(geom = 'bar', fun.data = mean_se) +
  stat_summary(geom = 'errorbar', fun.data = mean_se, width = 0.2) +
  geom_point() +
  scale_fill_brewer(palette = "Set2", guide = FALSE) +
  scale_y_continuous(limits = c(0,1), breaks = seq(0,1,.25)) +
  labs(x = 'Age Group', y = 'False Alarm Rate', title = 'False Alarm Rates', subtitle = 'As a function of Age Group')
```
