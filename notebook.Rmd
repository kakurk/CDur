...
Using `emmeans`, calculating the one sample t-test of each mean in the graph above with zero:  
...
require(emmeans)
emmeans(m, ~ enctype + AgeGroup) -> EMMEANS.DF

contrast(EMMEANS.DF, method = 'poly', by = 'AgeGroup')
```

Kyle's attempt at comparing the linear and quadratic contrasts within each Age Group:  

```{r}
# Is the linear effect statistically different from the quadratic effect?

contrast(EMMEANS.DF, method = list(linear = c(-1, 0, 1),
                                   quadratic = c(1, -2, 1),
                                   linear_vs_quadratic = c(0, -2, 2)), 
         by = 'AgeGroup')
```
...
contrast(EMMEANS.DF, interaction = c('poly', 'consec'))
```
...
regression.model <- lm(DF.effect ~ enctype*composite, 
                       data = df.DFeffects,
...
A series of regressions within each cell of our experimental design:  
...
A series of bonferroni correct T-tests of each mean presented above with zero:  
...
A test of the linear and quadratic trends within each Age Group:

```{r message=FALSE}
# Is there a linear or quadratic effect for YAs and OAs?

require(emmeans)

emmeans(m, ~ enctype + AgeGroup) -> EMMEANS.RE

contrast(EMMEANS.RE, method = 'poly', by = 'AgeGroup')
```

Kyle's attempt at contrasting the linear an quadratic effects above:  
...
Is the linear/quadratic effect of Cue Duration different between the Age Groups? 

```{r}
# Is there a difference in the effects between YAs and OAs?

contrast(EMMEANS.RE, interaction = c('poly', 'consec'))
```
...
```{r}
# Just for Older adults, is there a significant interaction between cue duration and composite score?

regression.model <- lm(Rec.effect ~ enctype*composite, 
                       data = df.RecEffects, 
...
A simple linear regression run within each cell of our experimental design:  
