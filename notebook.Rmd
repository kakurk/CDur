...
RateTbl %>%
  ...
  kable(col.names = c('Age Group', '', '1 sec', '3 sec', '5 sec', '1 sec', '3 sec', '5 sec'), 
        caption = 'Table 2: Behavioral Response Rates') %>%
  kable_classic() %>%
...
MissRates %>% 
  ...
  theme_classic() +
  labs(caption = 'Points represent the mean. Bars represent +/- SEM', 
       y = 'Miss Rate', 
       x = 'Cue Duration',
       title = 'Miss Rates', 
...
ANOVA.fit$ANOVA %>%
  kable(caption = 'Table S2: Rec Rate ~ AgeGroup, Cue Type, and Cue Duration ANOVA') %>%
  kable_classic(full_width = FALSE) %>%
  footnote(general = 'DFn indicates degrees of freedom numerator. DFd indicates degrees of freedom denominator. SSn indicates sum of squares numerator. SSd indicates sum of squares denominator. ges indicates generalized eta-squared.') -> TblS2

save_kable(TblS2, file = 'Supplemental_Table2.jpeg')
