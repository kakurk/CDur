...
df.demographics %>%
  mutate(AgeGroup = str_extract(string = SUBJECT, pattern = '^[yo]'),
         AgeGroup = factor(AgeGroup, levels = c('y', 'o'), labels = c('Young', 'Old'))) %>%
  group_by(AgeGroup) %>%
  mutate_at(CompositeVars, scale) %>%
  ungroup() %>%
  mutate(COMPOSITEZSCORE = pmap_dbl(select(., one_of(CompositeVars)), lift_vd(mean))) -> df.demographics
```
...
df.demographics %>%
  mutate(AgeGroup = factor(ifelse(AGE > 30, 'Old Adults', 'Younger Adults'))) %>% # create an AgeGroup variable
...
df.DFeffects %>%
  ...
  ggplot(aes(x = enctype, y = meanDF.effect, fill = enctype)) + 
  geom_bar(stat="identity") + 
  geom_errorbar(aes(ymin = meanDF.effect - semDF.effect, ymax = meanDF.effect + semDF.effect), width = 0.2) +
  scale_fill_brewer(palette = "Set1") +
  ggtitle("Directed Forgetting Effect Across Cue Durations") +
...
DF.onesecond   <- t.test(df.DFeffects$DF.effect[df.DFeffects$enctype == '1 Second'], 
                         mu = 0, alternative = "greater")
DF.threesecond <- t.test(df.DFeffects$DF.effect[df.DFeffects$enctype == '3 Seconds'], 
                         mu = 0, alternative = "greater")
DF.fivesecond  <- t.test(df.DFeffects$DF.effect[df.DFeffects$enctype == '5 Seconds'], 
                         mu = 0, alternative = "greater")
```

**Conclusions:**  

1.) Collapsed across `AgeGroup`, the `1 Second` Cue Duration Condition showed a Directed Forgetting Effect that was signifcantly greater than 0 (t(`r DF.onesecond$parameter`) = `r round(DF.onesecond$statistic, digits = 2)`, p = `r round(DF.onesecond$p.value, digits = 3)`).  

2.) Collapsed across `AgeGroup`, the `3 Second` Cue Duration Condition showed a Directed Forgetting Effect that was significantly greater than 0 (t(`r DF.threesecond$parameter`) = `r round(DF.threesecond$statistic, digits = 2)`, p < `r if(DF.threesecond$p.value < .001) (.001)`).  

3.) Collapsed across `AgeGroup`, the `5 Second` Cue Duration Condition showed a Directed Forgetting Effect that was significantly greater than 0 (t(`r DF.fivesecond$parameter`) = `r round(DF.fivesecond$statistic, digits = 2)`, p < `r if(DF.fivesecond$p.value < .001) (.001)`).  
...
df.DFeffects %>%
  group_by(AgeGroup) %>%
  summarize(meanDF.effect = mean(DF.effect), semDF.effect = sd(DF.effect)/sqrt(n())) %>%
  ggplot(aes(x = AgeGroup, y = meanDF.effect, fill = AgeGroup)) + 
  geom_bar(stat="identity") + 
  geom_errorbar(aes(ymin = meanDF.effect - semDF.effect, ymax = meanDF.effect + semDF.effect), width = 0.2) +
  scale_fill_brewer(palette = "Set2") +
  scale_y_continuous(limits = c(0, .2)) +
  ggtitle("Directed Forgetting Effect Across Age Group") +
...
DF.YA.test   <- t.test(df.DFeffects$DF.effect[df.DFeffects$AgeGroup == 'Young'], 
                       mu = 0, alternative = "greater")
DF.OA.test   <- t.test(df.DFeffects$DF.effect[df.DFeffects$AgeGroup == 'Old'], 
                       mu = 0, alternative = "greater")
```

**Conclusions:**  

1.) Collapsed across `enctype`, `Young Adults` showed a Directed Forgetting Effect that was signifcantly greater than 0 (t(`r DF.YA.test$parameter`) = `r round(DF.YA.test$statistic, digits = 2)`, p < `r if(DF.OA.test$p.value < .001) (.001)`).  

2.) Collapsed across `enctype`, the `Older Adults` showed a Directed Forgetting Effect that was signficantly greater than 0 (t(`r DF.OA.test$parameter`) = `r round(DF.OA.test$statistic, digits = 2)`, p < `r if(DF.OA.test$p.value < .001) (.001)`).  
...
df.DFeffects %>%
  ...
  summarize(meanDF.effect = mean(DF.effect), semDF.effect = sd(DF.effect)/sqrt(n())) %>%
  ggplot(aes(x = enctype, y = meanDF.effect, fill = AgeGroup)) + 
  geom_bar(stat="identity", position = "dodge") + 
  geom_errorbar(aes(ymin = meanDF.effect - semDF.effect, ymax = meanDF.effect + semDF.effect), width = 0.2, position = position_dodge(.9)) + 
  scale_fill_brewer(palette = "Set2") + 
