...
summary(EMMEANS.DF, digits = 2, infer = T, adjust = 'bonferroni')
```
...
regression.model <- aov(DF.effect ~ AgeGroup*enctype*composite, data = df.DFeffects)
...
ggplot(df.RecEffects, aes(y = Rec.effect, x = composite, colour = AgeGroup)) + 
  ...
  geom_point() + 
  facet_grid(AgeGroup ~ enctype) + 
  geom_smooth(method = "glm", se = FALSE, formula = y ~ x, colour = "navyblue", linetype = "longdash") +
