...
# ANOVA
ANOVA <- aov(Rec.effect ~ enctype + Error(subject/enctype), data = df.RecEffects, subset = df.RecEffects$AgeGroup == 'Old')

# Summarize, break into between and repeated factors
summary(ANOVA, split = list(enctype = list(Linear = 1, Quad = 2)), expand.split = T)
```

**Cue Duration ANOVA (Only YAs)**
...
# ANOVA
ANOVA <- aov(Rec.effect ~ enctype + Error(subject/enctype), data = df.RecEffects, subset = df.RecEffects$AgeGroup == 'Young')
...
ANOVA.fit$aov %>%
  ...
  magrittr::extract2(1) %>%
  as_tibble(rownames = 'Effect') %>%
  kable(caption = 'Table 1: Miss Rates ~ Cue Duration -- Young TBR items only') %>%
  kable_classic() %>%
  add_indent(c(2,3)) %>%
...
ANOVA.fit$aov %>%
  ...
  magrittr::extract2(1) %>%
  as_tibble(rownames = 'Effect') %>%
  kable(caption = 'Table 3: Miss Rates ~ Cue Duration ANOVA -- Old TBR items only') %>%
  kable_classic() %>%
  add_indent(c(2,3)) %>%
